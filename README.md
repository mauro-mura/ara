...
| `failover/FailoverExample` | stub | LLM + embedding failover: a `PRIMARY_ONLY` agent dies on a 503, `FAILOVER` survives via its fallback, and a 401 aborts the chain instead of being masked — the same two outcomes for an `EmbeddingEndpointPool` feeding `InMemoryDocumentStore`. A 4th agent kept alive across 6 runs shows the per-candidate circuit breaker: after 3 consecutive failures the dead primary is open and skipped entirely, so an outage stops costing a timeout per request |
